...
def generate_alert(start_date, end_date):
    filtered_df = forecast_df[
        (forecast_df['Date'] >= start_date) & (forecast_df['Date'] <= end_date)
    ]
    
    heavy_days = filtered_df[filtered_df['Rain_Category'].isin(['Heavy rain', 'Very heavy rain', 'Extremely heavy rain'])]
    
    if not heavy_days.empty:
        alert_dates = heavy_days['Date'].dt.strftime('%Y-%m-%d').tolist()
        return f"⚠️ ALERT: Predicted heavy or extremely heavy rainfall on {', '.join(alert_dates)}. Please stay prepared!"
    else:
        return "✅ No heavy rainfall predicted during the selected period."
